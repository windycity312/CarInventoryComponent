...
class ReactCloudinaryUploader extends React.Component{
    state =  {
    ...
    handleClick = (ev) =>{
        ...
        try{
            var options = {
                cloud_name: self.props.cloudName,
                upload_preset: self.props.uploadPreset,
                multiple: true,
                returnJustUrl: true
            };
    ...
    render(){
        console.log('buttonCaption' + this.props.buttonCaption);
...
const CustomImageItem = ({image, deleteImage}) => {
    const [fontSize, setFontSize] = useState('16px');
    return (
        <div style={{position:'relative'}}>
            <img src={image} style={{width:'210px', height: '170px', marginBottom: '15px', marginRight: '30px'}}/>
            <Icon type="delete" style={{ fontSize: fontSize, color: '#ffffff', position: 'absolute', top:'5px', right: '35px'}} theme="outlined" onMouseEnter={() => setFontSize('20px')}
                onMouseLeave={() => setFontSize('16px')} onClick={() => deleteImage(image)}/>
        </div>
    )
};

const CustomImageView = ({images, deleteImage}) => {
    return (
        <div style={{display: 'flex', flexWrap: 'wrap'}}>
            {
                images.map((image,index)=>(
                    <CustomImageItem image={image} deleteImage={deleteImage}/>
                ))
            }
        </div>
    )
...
export const AddVehicle = ({ addVehicle, history }) => {
    ...
    const deleteImage = (delete_image) => {
        const {images} = formData;
        images.splice(images.indexOf(delete_image), 1);
        setFormData({
            ...formData,
            images: images
        })
            ...
            <AddVehicleCard >
                <Form onSubmit={onSubmit} >
                    ...
                    <Form.Item label="IMAGES">
                        <CustomImageView images={formData.images} deleteImage={deleteImage}/>
                        <ReactCloudinaryUploader cloudName='empire-motors' uploadPreset='fsd1weit' multiple={true} addImages={addImages}/>
                    </Form.Item>
